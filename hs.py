...
class StratumClient:
    ...
    def subscribe(self):
        try:
            ...
            if 'result' in response and isinstance(response['result'], list) and len(response['result']) >= 3:
                self.extranonce1 = response['result'][1]
                self.extranonce2_size = response['result'][2]
                self.subscribed = True
                logging.info("Subscribed to stratum server")
            else:
                logging.error("Subscribe response does not contain expected data")
                raise Exception("Subscribe response does not contain expected data")
    ...
    def request_job(self):
        try:
            ...
            logging.error(f"Request job error: {str(e)}")
            raise
    ...
    def handle_job(self, job):
        try:
            if len(job) >= 9:
                ...
                logging.error("Job response does not contain expected data")
                raise Exception("Job response does not contain expected data")
        
        except Exception as e:
            logging.error(f"Handle job error: {str(e)}")
            raise
...
if __name__ == "__main__":
    ...
    for i in range(num_threads):
        t = threading.Thread(target=mine_thread, args=(i, client))
        threads.append(t)
        t.start()
